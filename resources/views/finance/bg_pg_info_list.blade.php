                        ...
                        <tbody>
                            @foreach ($m_bg_pg as $key => $raw)
                                <tr>
                                    ...
                                    <td>
                                        @if ($raw->beneficiary_type == 1)
                                            {{ 'BREB' }}
                                        @elseif ($raw->beneficiary_type == 2)
                                            {{ 'BPDB' }}
                                        @elseif ($raw->beneficiary_type == 3)
                                            {{ 'DESCO' }}
                                        @elseif ($raw->beneficiary_type == 4)
                                            {{ 'DPDC' }}
                                        @elseif ($raw->beneficiary_type == 5)
                                            {{ 'NESCO' }}
                                        @elseif ($raw->beneficiary_type == 6)
                                            {{ 'WZPDCL' }}
                                        @endif
                                    </td>
                                    <td>{{ $raw->bank_name }} </td>
                                    ...
                                    <td>{{ $raw->expense }}</td>
                                    <td>
                                        @if ($raw->nature_bgpg == 1)
                                            {{ 'BG' }}
                                        @elseif ($raw->nature_bgpg == 2)
                                            {{ 'PG' }}
                                        @elseif ($raw->nature_bgpg == 3)
                                            {{ 'BB' }}
                                        @endif
                                    </td>
                                    <td>
                                        @php
                                            $m_employee_info = DB::table('pro_employee_info')
                                                ->where('company_id', $raw->ref_id)
                                                ->first();
                                        @endphp
                                        @isset($m_employee_info)
                                            {{ $m_employee_info->employee_name }}
                                        @endisset
                                    </td>
                                    <td>{{ $raw->remarks }}</td>
