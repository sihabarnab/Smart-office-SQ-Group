...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Indent for Approved</h1>
                </div><!-- /.col -->
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('CompanyFundReqApprovedList') }}" method="post">
                        @csrf
                        <div class="row mb-2">
                            <div class="col-12">
                            ...
                            <table id="data1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    @foreach ($m_fund_req_master as $key => $value)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $value->fund_req_master_id }}<br>{{ $value->fund_req_date }}</td>
                                            <td>{{ $value->company_name }}</td>
                                            <td>{{ $value->req_form }}</td>
                                            <td>{{ $value->req_to }}</td>
                                            <td>{{ $value->employee_id }} <br> {{ $value->employee_name }}</td>
                                            <td>{{ $value->first_check }} <br> {{ $value->first_name }}</td>
                                            <td>{{ $value->second_check }} <br> {{ $value->second_name }}</td>
                                            <td><a href="{{ route('FundReqApproved',[$value->fund_req_master_id,$value->company_id]) }}"
