...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Requsition List</h1>
                </div><!-- /.col -->
    ...
    <div class="container-fluid" id='rr'>
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <table id="rpt_requsition_list" class="table table-bordered table-striped table-sm">
                            <thead>
                                <tr>
...
@section('script')
    ...
    <script>
        ...
        function getrrlist(company_id, form, to) {
            ...
            var k = 1;
            $.ajax({
                url: "{{ url('/get/rpt_requsition_list') }}/" + company_id + '/' + form + '/' + to,
                type: "GET",
                dataType: "json",
                success: function(data) {
                    if (data) {
                        $('#loader').hide();
                        if ($.fn.DataTable.isDataTable("#rpt_requsition_list")) {
                            $('#rpt_requsition_list').DataTable().clear().destroy();
                        }
                    }
                    $('#rpt_requsition_list').dataTable({
                        "responsive": true,
                        "lengthChange": false,
                        "autoWidth": false,
                        dom: 'Blfrtip',
                        ...
                        "columns": [{
                            ...
                            },
                            {
                                data: null,
                                render: function(data, type, full) {
                                    return '<a target="_blank" href="{{ url('/') }}/inventory/rpt_requsition_details/' +
                                        data.mrm_no + '/' + data.company_id +
                                        '">view / print</a>';
                                }
                            },
                        ], // end colume
