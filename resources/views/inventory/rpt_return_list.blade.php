    ...
    <div class="container-fluid" id ='rr'>
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <table id="rpt_return_list" class="table table-bordered table-striped table-sm">
                            <thead>
                                <tr>
                                    <th>SL No.</th>
                                    <th>Project</th>
                                    <th>Section</th>
                                    <th>Return No/Date</th>
                                    <th>Voucher No</th>
                                    <th>Remark</th>
                                    <th></th>
                                </tr>
...
@section('script')
    ...
    <script>
        ...
        function getrrlist(company_id, form, to) {
            ...
            var k = 1;
            $.ajax({
                url: "{{ url('/get/rpt_return_list') }}/" + company_id + '/' + form + '/' + to,
                type: "GET",
                dataType: "json",
                success: function(data) {
                    if (data) {
                        $('#loader').hide();
                        if ($.fn.DataTable.isDataTable("#rpt_return_list")) {
                            $('#rpt_return_list').DataTable().clear().destroy();
                        }
                    }
                    $('#rpt_return_list').dataTable({
                        "responsive": true,
                        "lengthChange": false,
                        "autoWidth": false,
                        dom: 'Blfrtip',
                        ...
                        "columns": [{
                            ...
                            },
                            {
                                data: null,
                                render: function(data, type, full) {
                                    return data.return_master_no + '<br>' + data
                                        .return_date;
                            ...
                            },
                            {
                                data: null,
                                render: function(data, type, full) {
                                    return '<a target="_blank" href="{{ url('/') }}/inventory/rpt_return_details/' +
                                        data.return_master_no + '/' + data.company_id +
                                        '">view / print</a>';
                                }
                            },
                        ], // end colume
