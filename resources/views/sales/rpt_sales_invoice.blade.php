    ...
    <div class="container-fluid" id='rr'>
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <table id="sales_invoice_list" class="table table-bordered table-striped table-sm">
                            <thead>
                                <tr>
                                    <th>SL No</th>
                                    <th>Invoice No.</th>
...
<script>
    ...
    <script>
        function getrrlist(company_id, form, to) {
            ...
            var k = 1;
            $.ajax({
                url: "{{ url('/get/sales/sales_invoice_list') }}/" + company_id + '/' + form + '/' + to+"/"+product_group,
                type: "GET",
                dataType: "json",
                success: function(data) {
                    if (data) {
                        // $('#loader').hide();
                        if ($.fn.DataTable.isDataTable("#sales_invoice_list")) {
                            $('#sales_invoice_list').DataTable().clear().destroy();
                        }
                    }
                    $('#sales_invoice_list').dataTable({
                        "responsive": true,
                        "lengthChange": false,
                        "autoWidth": false,
                        dom: 'Blfrtip',
                        ...
                        "columns": [{
                            ...
                            },
                            {
                                data: null,
                                render: function(data, type, full) {
                                    return data.sim_id+'<br>'+data.sim_date;
                                }
                            },
                            {
                                "data": "customer_name"
                            ...
                            },
                            {
                                data: null,
                                render: function(data, type, full) {
                                    return '<a target="_blank" href="{{ url('/') }}/sales/rpt_sales_invoice_view_regular/' +
                            ...
                            },
                            {
                                data: null,
                                render: function(data, type, full) {
                                    return '<a target="_blank" href="{{ url('/') }}/sales/rpt_sales_invoice_view/' +
                                        data.sim_id + '/' + data.company_id +
                                        '">view/print</a>';
                                }
                            },
