                        ...
                        <table id="quotation_list" class="table table-bordered table-striped table-sm">
                            ...
                            @endphp
                              ...
                              @php
                                 ...
                                 $data = DB::table('pro_quotation_master')->where('company_id',$company_id)->where('status', 2)->get();
                              @endphp
                                  @foreach ($data as $row)
                                      <tr>
                                        <td>{{$x++}} </td>
                                        ...
                                        <td><a href="{{route('rpt_quotation_view',[$row->quotation_id,$row->company_id])}}">view/Print</a></td>
                                      </tr>
                                  @endforeach
                              @endforeach --}}
                            </tbody>
...
@section('script')
    ...
    <script>
        function getrrlist(company_id, form, to) {
            ...
            var k = 1;
            $.ajax({
                url: "{{ url('/get/sales/quotation_list') }}/" + company_id + '/' + form + '/' + to,
                type: "GET",
                dataType: "json",
                    ...
                    $('#quotation_list').dataTable({
                        ...
                        "columns": [{
                            ...
                            },
                            {
                                data: null,
                                render: function(data, type, full) {
                                    return '<a target="_blank" href="{{ url('/') }}/sales/rpt_quotation_view/' +
                                        data.quotation_id + '/' + data.company_id +
                                        '">view/print</a>';
                                }
                            },
                        ], // end colume
