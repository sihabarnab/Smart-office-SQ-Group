...
//policy
    ...
    public function HrmPolicySubEdit($id2)
    {
        ...
        $user_company = DB::table("pro_user_company")
        ->join("pro_company", "pro_user_company.company_id", "pro_company.company_id")
        ->select("pro_user_company.*",)
        ->Where('employee_id',$m_user_id)
        ->get();

        $m_att_policy=DB::table('pro_att_policy')
        ->leftjoin("pro_company", "pro_att_policy.company_id", "pro_company.company_id")
        ->select(
            "pro_att_policy.*",
            "pro_company.company_name",
        )
        ->where('att_policy_id',$id2)
        ->first();

        $ci_att_policy_sub=DB::table('pro_att_policy_sub')
        ->where('pro_att_policy_sub.att_policy_id',$id2)
        ->where('pro_att_policy_sub.valid','1')
        ->get();

        $m_yesno=DB::table('pro_yesno')->Where('valid','1')->orderBy('yesno_id','asc')->get(); //query builder


        return view('hrm.policy_sub',compact('user_company','m_att_policy','ci_att_policy_sub','m_yesno'));
    }

    public function HrmPolicySubOk(Request $request)
    {

        $m_company_id=$request->txt_company_id;
        $m_att_policy_id=$request->txt_att_policy_id;
        $m_att_policy_sub_id=$request->txt_att_policy_sub_id;
        ...
        DB::table("pro_att_policy_sub")
            ->where('att_policy_sub_id', '=', $m_att_policy_sub_id)
            ->update([
                'in_time' => $request->txt_in_time,
                'out_time' => $request->txt_out_time,
                'grace_time' => $request->txt_grace_time,
...
//holiday
    ...
    public function hrmbackholiday()
    {
        ...
        ->Where('valid','1')
        ->Where('holiday_status','2')
        ->orderBy('placeofposting_id','asc')
        ->get();
       
        $data=DB::table('pro_holiday')
        ->leftjoin("pro_placeofposting", "pro_holiday.placeofposting_id", "pro_placeofposting.placeofposting_id")
        ->select(
            "pro_holiday.*",
            "pro_placeofposting.placeofposting_name",
        )
        ->Where('pro_holiday.valid','1')
        ->where('pro_holiday.holiday_year',$m_holiday_year)
        ->orderBy('pro_holiday.holiday_id', 'ASC')
        ->get(); //query builder
        
        return view('hrm.holiday',compact('data','user_company','m_placeofposting'));
    }
    ...
    public function hrmbackholidaystore(Request $request)
    {
        $rules = [
            ...
            // 'cbo_sub_posting' => 'required',
            'txt_holiday_name' => 'required',
            'txt_holiday_date' => 'required',
                ];
        $customMessages = [
            // 'cbo_company_id.required' => 'Company is required.',
            // 'cbo_posting.required' => 'Posting is required.',
            // 'cbo_sub_posting.required' => 'Sub Posting is required.',
        ...
        if ($request->cbo_posting=='0'){
            $abcd = DB::table('pro_holiday')
            ->where('holiday_name', $request->txt_holiday_name)
            ->where('holiday_year', $m_holiday_year)
            // ->where('company_id', $request->cbo_company_id)
            // ->where('placeofposting_id', $request->cbo_posting)
            // ->where('placeofposting_sub_id', $request->cbo_sub_posting)
            ->first();
            //dd($abcd);
        } else {
            $abcd = DB::table('pro_holiday')
        ...
        $data['placeofposting_sub_id']=$request->cbo_sub_posting;
...
//randome password
        ...
        $hash_password = password_hash($pass,  
          PASSWORD_DEFAULT);
          if($request->cbo_yesno_id == 2){
            DB::table('users')->where('emp_id',$request->cbo_employee_id)->update([
                'password'=>$request->cbo_yesno_id,
                ]);
          }
            ...
            $txt_app_day=$ci_pro_leave_config->app_day;
                ...
                if ($ci_pro_cl_policy)
                {
                    ...
                    if ($ci_leave_info_master===null)
                    {
                        ...
                        foreach ($ci_level_step as $key => $row) {
                ...
                // DB::table("$m_table1")->insert($data);
                        ...
                        $massage="$request->txt_user_id | $txt_employee_name Leave Application";
                        DB::table('pro_alart_massage')->insert([
                        'message_id'=>$request->txt_user_id,
                        'report_id'=>$row->report_to_id,
                        'massage'=>$massage,
                        'valid'=>1,
...
// $m_user_id=Auth::user()->emp_id;
 ...
 public function hrmbackleave_approval_others(){
        ...
        ->Where('pro_leave_info_master.leave_type_id','>','0')
...
//movement_approval
    ...
    public function hrmmovement_approval()
    {
      ...
      $ci_report=DB::table('pro_level_step')
        ...
        // ->Where('status','1')
        ->Where('pro_level_step.report_to_id',$m_user_id)
        // ->Where('pro_level_step.employee_id','00002059')
        ->orderBy('pro_level_step.employee_id','DESC')
        ->get();
...
public function HrmMoveApprovalOthers(){
    ...
    public function move_file_store(Request $request,$id){
        ...
        $txt_doc_file = $request->file('txt_file');
            ...
            ->update($data);
                ...
                // 'approved_level'=>$m2_level_step1,
                'status'=>$m_status,
                'g_late_form'=>$m_late_inform_master->late_form,
    ...
    public function hrmbackrptovement()
    {
        $m_user_id=Auth::user()->emp_id;
        $user_company = DB::table("pro_user_company")
        ->leftjoin("pro_company", "pro_user_company.company_id", "pro_company.company_id")
        ->select("pro_company.company_name",'pro_company.company_id')
        ->Where('employee_id',$m_user_id)
        ->get();
        $m_placeofposting=DB::table('pro_placeofposting')
        ...
        ->select('pro_desig.desig_id','pro_desig.desig_name')
        ->Where('valid','1')
        ->orderBy('desig_id', 'desc')
        ->get();
        return view('hrm.rpt_movement',compact('user_company','m_placeofposting','m_desig'));
    }

    public function hrmbackrptovementList(Request $request)
    {
        ...
        $data=DB::table('pro_late_inform_master')
        // ->Where('employee_id',$m_user_id)
        ->leftJoin('pro_employee_info','pro_late_inform_master.employee_id','pro_employee_info.employee_id')
        ->select('pro_late_inform_master.*','pro_employee_info.employee_name')
        ->Where('pro_late_inform_master.company_id',$request->cbo_company_id)
        ->Where('pro_late_inform_master.placeofposting_id',$request->cbo_posting)
        ->Where('pro_late_inform_master.valid','1')
        ->orderBy('pro_late_inform_master.late_inform_master_id','DESC');

        if($request->txt_from_date && $request->txt_to_date && $request->cbo_employee_id == null)
        {
            $m_movement=$data->whereBetween('pro_late_inform_master.entry_date',[$request->txt_from_date,$request->txt_to_date])->get();
        }
        elseif($request->txt_from_date == null && $request->txt_to_date == null && $request->cbo_employee_id)
        {
            $m_movement=$data->where('pro_late_inform_master.employee_id',$request->cbo_employee_id)->get();
        }else{
            $m_movement=$data->get();
        }
        ...
        $company_id = $request->cbo_company_id;
        $placeofposting_id = $request->cbo_posting;
        $placeofposting_sub_id = $request->cbo_sub_posting;
        $desig_id = $request->cbo_desig_id;
        $form = $request->txt_from_date;
        $to = $request->txt_to_date;
        //

       return view('hrm.rpt_movement_list', compact('m_movement','user_company','m_placeofposting','m_desig','company_id','placeofposting_id','placeofposting_sub_id','desig_id','form','to'));
 ...
 public function HrmAllPolicyChange()
    {
        ...
        return view('hrm.emp_all_policy_change',compact('user_company'));

    }

    public function HrmAllPolicyChangeStore(Request $request)
    {
        $rules = [
            'cbo_company_id' => 'required|integer|between:1,1000',
            'cbo_posting' => 'required|integer|between:1,1000',
            'cbo_sub_posting' => 'required|integer|between:1,1000',
        ...
        $customMessages = [
            ...
            'cbo_company_id.between' => 'Select Company.',
            'cbo_posting.required' => 'Select Posting.',
            'cbo_posting.integer' => 'Select Posting.',
            'cbo_posting.between' => 'Select Posting.',
            'cbo_sub_posting.required' => 'Select Sub Posting.',
            'cbo_sub_posting.integer' => 'Select Sub Posting.',
            'cbo_sub_posting.between' => 'Select Sub Posting.',
        ...
        ->Where('placeofposting_sub_id',$request->cbo_sub_posting)
        ->first();
        $txt_sub_placeofposting_name=$m_sub_placeofposting->sub_placeofposting_name;


        return redirect(route('emp_all_policy_change'))->with('success',"$txt_company_name | $txt_placeofposting_name | $txt_sub_placeofposting_name Policy Update Successfully!");
...
//attendance_process
    ...
    $rules = [
        ...
        $m_year1=date("Y",strtotime($m_atten_date));

        $m_table="pro_attendance_$m_month$m_year";
        $m_emp_day_policy_table = "pro_emp_day_policy_$m_year1$m_month";
...
// dd($m_table);
        if (Schema::hasTable("$m_table"))
        {
            ...
            if ($ci_attendance>0)
            {
                ...
                if (Schema::hasTable("$m_emp_day_policy_table"))
                {

                    $m_employee_info = DB::table("$m_emp_day_policy_table")
                    ->where('valid','1')
                    // ->where('company_id','21')
                    // ->where('placeofposting_id','80')
                    // ->where('placeofposting_sub_id','4')
                    ->where('attn_date',$m_atten_date)
                    ->orderBy('employee_id','asc')
                    ->get();

                    if (isset($m_employee_info))
                    {
                        ...
                        foreach ($m_employee_info as $row_emp_info){
                            ...
                            $m_in_time=$row_emp_info->r_in_time;
                            $m_in_time_graced=$row_emp_info->p_in_time;
                            $m_out_time=$row_emp_info->p_out_time;
                            $m_company_id=$row_emp_info->company_id;
                            $m_placeofposting_id=$row_emp_info->placeofposting_id;
                            $m_placeofposting_sub_id=$row_emp_info->placeofposting_sub_id;
                            ...
                            if($m_placeofposting_id=='8' || $m_placeofposting_id=='80')
                            {
                                ...
                                // ->Where('placeofposting_sub_id',$m_placeofposting_sub_id)
                                ->first();
                            ...
                            $sts="A";
                                ...
                                $data['placeofposting_sub_id']=$m_placeofposting_sub_id;
        ...
        // dd($data);
                            ...
                            // DB::table("$m_table")->where('employee_id',$m_employee_id)->where('attn_date',$m_atten_date)->update([
                ...
                return redirect()->back()->withInput()->with('warning','Day Shift Table Not Found');
                }
//end of step 2
            ...
            $table->integer('placeofposting_sub_id')->length(11);
...
//company & posting wise attendance_process
    ...
    public function HrmAttenProcessComPosting()
    {
        ...
        ->Where('valid','1')
        ->Where('sub_posting_status','2')
        ->orderBy('placeofposting_id','asc')
        ->get(); //query builder

        return view('hrm.attendance_process_com_posting',compact('user_company','ci_placeofposting'));

    }

    //Attendance Process 01
    public function HrmAttenProcessComPostingStore(Request $request)
    {
    $rules = [
            'cbo_company_id' => 'required',
            'txt_from_date' => 'required',
        ...
        $customMessages = [
            ...
            'cbo_company_id.required' => 'Company is required.',
            'txt_from_date.required' => 'Attendance From Date is required.',
            'txt_to_date.required' => 'Attendance to Date is required.',
        ...
        $m_entry_time=date("H:i:s",$mentrydate);
        $m_valid='1';
        $m_atten_date1=$request->txt_from_date;
        $m_atten_date2=$request->txt_to_date;

        $m_month=date("m",strtotime($m_atten_date1));
        $m_year=date("y",strtotime($m_atten_date1));
        $m_year1=date("Y",strtotime($m_atten_date1));
        ...
        $diff = (strtotime($m_atten_date2) - strtotime($m_atten_date1));
        $tot_days = floor($diff / (1*60*60*24)+1);

        for($d=0; $d<$tot_days; $d++)

        {
            $m_atten_date = date('Y-m-d', strtotime($m_atten_date1.' + '.$d.' days'));

        if (Schema::hasTable("$m_table"))
        {
            
            if ($request->cbo_placeofposting_id==null)
            {
                $ci_attendance=DB::table("$m_table")
                ->where('attn_date',$m_atten_date)
                ->where('valid','1')
            ...
            if ($ci_attendance>0)
            {
                ...
                if ($request->cbo_placeofposting_id==null)
                {
                    ...
                    $txt_placeofposting_name="(except Bhabanipur, Sripur)";
...
// dd($m_employee_info);
                ...
                // return redirect()->back()->with('success',"$txt_company_name $txt_placeofposting_name $request->txt_atten_date Data Process 01 Successfully!");
...
// dd("$m_employee_id - $mm_shift_status - $m_in_time - $m_p_in_time - $m_out_time - $timeDiffin");
    ...
    public function HrmAttenProcessComPosting_02()
    {
        ...
        return view('hrm.attendance_process_com_posting_02',compact('user_company','ci_placeofposting'));

    }

    public function HrmAttenProcessComPostingStore_02(Request $request)
    {
    $rules = [
            'cbo_company_id' => 'required',
            'txt_from_date' => 'required',
        ...
        for($d=0; $d<$tot_days; $d++)
        {
            ...
            if ($ci_attendance>0)
            {
                ...
                return redirect()->back()->withInput()->with('warning',"$txt_company_name Attendance Process Allready Done");
            } else {
    ...
    // dd("$m_employee_id - $mm_shift_status - $m_in_time - $m_p_in_time - $m_out_time - $timeDiffin");
        ...
        return redirect()->back()->with('success',"$txt_company_name $txt_placeofposting_name $request->txt_atten_date Data Process 02 Successfully!");
...
//leave_process
    ...
    public function hrmbackleave_process()
    {
        ...
        return view('hrm.leave_process',compact('user_company','ci_placeofposting'));
...
// dd($ci_pro_attendance);
        ...
        {
            return redirect()->back()->withInput()->with('warning',"$txt_company_name $txt_month Data Not Found");  
...
// dd("$m_employee_id -- $m_leave_date");
            ...
            'process_status'=>$m_process_status,
              ...
              return redirect()->back()->withInput()->with('success'," $txt_company_name $txt_month Leave Process");
            }//if ($m_leave_sql===null)
        }//if ($ci_pro_attendance===null)
...
//movement_process
    ...
    public function hrmbackmovement_process()
    {
        ...
        return view('hrm.movement_process',compact('user_company','ci_placeofposting'));
            ...
            if ($m_move_sql_01=='0')
            {
              return redirect()->back()->withInput()->with('warning',"$txt_company_name $txt_month Data Not Found");  
                ...
                ->update([
                  ...
                  return redirect()->back()->withInput()->with('success',"$txt_company_name $txt_month Movement Process");  
...
//early_process
    ...
    public function HrmEarlyProcess()
    {
        ...
        return view('hrm.early_process',compact('user_company','ci_placeofposting'));
    }
            ...
            if ($m_move_sql_01=='0')
            {
              return redirect()->back()->withInput()->with('warning',"$txt_company_name $txt_month Data Not Found");  
                ...
                ->update([
                  ...
                  return redirect()->back()->withInput()->with('success',"$txt_company_name $txt_month Early Process");  
...
// dd("$m_pro_attendance");
        ...
        $m_r_in_time=$row_pro_attendance->r_in_time;
        $m_late_min=$row_pro_attendance->late_min;
...
// dd("$m_employee_id");
        ...
        $checktime=(strtotime($m_in_time)-strtotime($m_r_in_time));
        $checktimeDiffin = floor($checktime / 60);
        $m_abc='-30';
        if ($m_late_min>30)
        {
            $m_working_minuts = '0';
        ...
        return redirect()->back()->withInput()->with('success',"$txt_company_name $txt_month Working Time Process");  
...
//summary_process
    ...
    public function hrmbacksummary_process()
    {
        ...
        return view('hrm.summary_process',compact('user_company','ci_placeofposting','ci_summ_attn_master'));
    ...
    $rules = [
        ...
        // dd($request->cbo_placeofposting_id);
        if ($request->cbo_placeofposting_id=='0')
        {
            $m_summ_attn_master_01=DB::table('pro_summ_attn_master')
            ->Where('company_id',$request->cbo_company_id)
...
// dd($m_summ_attn_master);
        if ($m_summ_attn_master_01===null)
        {
            // dd($this->m_user_id);
            ...
            foreach ($m_attn_employee as $row_attn_employee)
            {
                ...
                $mm_psm_id=$m_employee_info->psm_id;
          ...
          return redirect()->back()->withInput()->with('success',"$txt_company_name $txt_month Summery Process Completed");  

        } else {

            $txt_summ_attn_master_id=$m_summ_attn_master_01->summ_attn_master_id;
          ...
          // return redirect()->back()->withInput()->with('warning','Data already exists!!');  
          return redirect()->back()->withInput()->with('warning',"$txt_company_name $txt_month Data Update Successfully !!!");  
        }//if ($m_summ_attn_master===null)

    }

    public function EmpDayShiftReport(Request $request)
    {
    $rules = [
            'cbo_company_id' => 'required|integer|between:1,100',
            'cbo_posting' => 'required',
        ...
        $diff = (strtotime($txt_last_date) - strtotime($txt_frist_date));
        $tot_days = floor($diff / (1*60*60*24)+1);
        // dd("$txt_month | $txt_frist_date | $txt_last_date | $tot_days");
        ...
        $ci_sub_placeofposting=DB::table('pro_sub_placeofposting')
        ->Where('placeofposting_sub_id',$request->cbo_sub_posting)
        ->first();
        $txt_sub_placeofposting_name=$ci_sub_placeofposting->sub_placeofposting_name;

        // dd("$txt_sub_placeofposting_name | );
        if ($request->cbo_sub_posting === '0')
        {
            return redirect()->back()->withInput()->with('warning','Sorry Sub Posting Required!!!!');
        } else {

            // dd($request->cbo_sub_posting);
            ...
            ->Where('pro_employee_info.company_id',$request->cbo_company_id)
            ->Where('pro_employee_info.placeofposting_id',$request->cbo_posting)
            ->Where('pro_employee_info.placeofposting_sub_id',$request->cbo_sub_posting)
            ->Where('pro_employee_info.working_status',1)
            ->Where('pro_employee_info.ss',1)
            ->Where('pro_employee_info.valid',1)
            ->get();
// dd($m_emp_day_policy);
            if ($m_employee_info == null)
            {
                return redirect()->back()->withInput()->with('warning','Data Not Found!!!!');
            } else {

                return view('hrm.rpt_emp_shift_report',compact('m_employee_info','tot_days','txt_company_name','txt_placeofposting_name','txt_sub_placeofposting_name','txt_year','month_name','txt_frist_date','txt_last_date'));
...
//payroll_process
    ...
    public function HrmDataQuery()
    {
        ...
        $m_user_posting = DB::table("pro_user_posting")
            ->leftjoin("pro_company", "pro_user_posting.company_id", "pro_company.company_id")
            ->leftjoin("pro_placeofposting", "pro_user_posting.placeofposting_id", "pro_placeofposting.placeofposting_id")
            ->select(
                "pro_user_posting.*", 
                "pro_company.company_name",
                "pro_placeofposting.placeofposting_name",
            )
            ->Where('pro_user_posting.employee_id',$m_user_id)
            ->Where('pro_user_posting.valid',1)
            ->get();


        return view('hrm.data_query',compact('user_company','m_user_posting'));
...
//summery_attendance report
    ...
    $rules = [
        ...
        if ($request->sele_placeofposting === null)
        {
            $m_summ_attn_master=DB::table('pro_summ_attn_master')
            ->Where('company_id',$request->cbo_company_id)
            ->Where('month',$txt_month1)
...
//employee_attendance report
    ...
    public function hrmbackemployee_attendance()
    {
        ...
        return view('hrm.employee_attendance',compact('user_company','m_user_posting'));
 ...
 public function hrmbackemp_attn_report(Request $request)
    ...
    public function HrmEmpAttenDate()
    {
        $m_user_id=Auth::user()->emp_id;
        ...
        return view('hrm.employee_attendance_date',compact('user_company'));

    }

 public function HrmEmpAttnDateReport(Request $request)
    {
    $rules = [
            'cbo_company_id' => 'required|integer|between:1,100',
            'txt_from_date' => 'required',
        ...
        $customMessages = [
            ...
            'cbo_company_id.integer' => 'Select Company.',
            'cbo_company_id.between' => 'Select Company.',
            'txt_from_date.required' => 'Select Month Year.',
        ...
        $m_pro_attendance=DB::table("$m_attendance")
        ->Where('company_id',$request->cbo_company_id)
        ->where('attn_date',$txt_frist_date)
        ->Where('process_status','>','4')
        ->Where('valid','1')
        ->count(); //query builder
      ...
      $ci_pro_attendance=DB::table("$m_attendance")
        ...
        ->get(); //query builder
          ...
          return view('hrm.emp_attn_date_report',compact('ci_pro_attendance','txt_frist_date','txt_company_id'));
        }
        } else {//if (Schema::hasTable("$m_attendance"))
            return redirect()->back()->withInput()->with('warning',"$month_name Data Not Found");
        }
...
//Daily Punch report
    ...
    public function hrmbackdaily_punch()
    {
        ...
        $m_user_posting = DB::table("pro_user_posting")
            ->join("pro_placeofposting", "pro_user_posting.placeofposting_id", "pro_placeofposting.placeofposting_id")
            ->select(
                "pro_user_posting.placeofposting_id",
                "pro_placeofposting.placeofposting_name"
            )
            ->Where('pro_user_posting.employee_id', $m_user_id)
            ->get();
        return view('hrm.daily_punch', compact('user_company', 'm_user_posting'));
    }
    ...
    public function hrmbackdaily_punch_report(Request $request)
    {
        ...
        $customMessages = [
            ...
            'txt_from_date.required' => 'Select form date.',
        ...
        if (Schema::hasTable("$m_tmp_log1")) {
            ...
            if ($m_tmp_log === null) {
                ...
                return view('hrm.emp_punch_report', compact('m_tmp_log', 'user_company','m_user_posting'));
            } //if ($m_from_date === null)
    ...
    public function hrmbackgetplaceofsubposting($placeofposting_id)
    {
        $m_user_id = Auth::user()->emp_id;
        $user_placeofsubposting_id = DB::table('pro_user_sub_posting')
            ->where('employee_id', $m_user_id)
            ->where('placeofposting_id', $placeofposting_id)
            ->where('valid', 1)
            ->pluck('placeofposting_sub_id');
        ...
        $data = DB::table('pro_sub_placeofposting')
            ...
            ->whereIn('placeofposting_sub_id', $user_placeofsubposting_id)
            ->Where('valid', '1')
    ...
    public function hrmbackgetemployee($placeofposting_id, $placeofposting_sub_id, $company_id)
    {
        if ($placeofposting_sub_id == 0) {
            $data = DB::table('pro_employee_info')
                ->where('working_status', '1')
                ->where('ss', '1')
                ...
                ->where('placeofposting_id', $placeofposting_id)
                ->get();
        } elseif ($placeofposting_sub_id > 0) {
            $data = DB::table('pro_employee_info')
                ->where('working_status', '1')
                ->where('ss', '1')
                ->where('company_id', $company_id)
                ->where('placeofposting_id', $placeofposting_id)
                ->where('placeofposting_sub_id', $placeofposting_sub_id)
                ->get();
        }
...
//Daily Attendance aaa
    ...
    public function hrmAaaReport(Request $request)
    {
        ...
        $m_atten_aaa = DB::table('pro_attendance_0424')
        ->where('company_id',$request->cbo_company_id)
...
//Sub Place Of Postion
    ...
    public function HrmSubPlaceOfPostingStore(Request $request)
    {
        ...
        return back()->with(['success'=>"$request->txt_sub_posting Add Successfully!",'placeofposting_id'=>$request->cbo_posting]);
        }
    }

    public function HrmSubPlaceOfPostingEdit($id)
    {
        $m_placeofposting=DB::table('pro_placeofposting')
        ->Where('valid','1')
        ->orderBy('placeofposting_id','asc')
       ...
       $m_sub_placeofposting = DB::table('pro_sub_placeofposting')->where('placeofposting_sub_id',$id)->first();

        return view('hrm.sub_placeofposting_edit',compact('m_placeofposting','m_sub_placeofposting'));
    }

    public function HrmSubPlaceOfPostingUpdate(Request $request,$id)
    {
        $rules = [
            'cbo_posting' => 'required',
            'txt_sub_posting' => 'required',
        ...
        $data['sub_placeofposting_name']=$request->txt_sub_posting;
        $data['user_id']=$m_user_id;
        $m_sub_placeofposting = DB::table('pro_sub_placeofposting')->where('placeofposting_sub_id',$id)->update($data);
        return redirect()->route('sub_placeofposting')->with(['success'=>"$request->txt_sub_posting update Successfully!",'placeofposting_id'=>$request->cbo_posting]);
    }
...
//Daily Shift Change
    ...
    public function HrmEmpDayShiftStore(Request $request)
    {
        $rules = [
            ...
            'cbo_company_id' => 'required',
            'cbo_posting' => 'required',
            'cbo_sub_posting' => 'required|integer|between:1,10000',
            'cbo_employee_id' => 'required',
            'sele_att_policy' => 'required',
        ];
    ...
    public function HrmEmpDayShiftFinal($attn_date,$id,$id1,$id2)
    {
        ...
        $month = date('m', strtotime($attn_date));
        $m_emp_day_policy_table = "pro_emp_day_policy_$year$month";
        if (Schema::hasTable("$m_emp_day_policy_table")) {
        } else {
            Schema::create("$m_emp_day_policy_table", function (Blueprint $table) {
                $table->increments('emp_day_policy_id');
                $table->date('attn_date');
                $table->integer('company_id')->length(11);
                $table->string('employee_id', 8);
                $table->integer('desig_id')->length(11);
                $table->integer('department_id')->length(11);
                $table->integer('section_id')->length(11);
                $table->integer('placeofposting_id')->length(11);
                $table->integer('placeofposting_sub_id')->length(11);
                $table->integer('att_policy_id')->length(2);
                $table->integer('att_policy_sub_id')->length(11);
                $table->time('r_in_time');
                $table->time('p_in_time');
                $table->time('p_out_time');
                $table->string('user_id', 8);
        ...
        foreach ($all_employee as $value)
        {
            ...
            if($m_policy_status==2)
            {
                ...
                $m_in_time_graced=$m_att_policy_sub->in_time_graced;
                $m_out_time=$m_att_policy_sub->out_time;
                $m_att_policy_sub_id=$m_att_policy_sub->att_policy_sub_id;
            ...
            $data['att_policy_sub_id'] = $m_att_policy_sub_id;
            $data['r_in_time'] = $m_in_time;
            $data['p_in_time'] = $m_in_time_graced;
            $data['p_out_time'] = $m_out_time;
            $data['user_id'] = $m_user_id;
            ...
            $check =  DB::table("$m_emp_day_policy_table")
            ->where('employee_id',$value->employee_id)
            ->where('attn_date',$attn_date)
            ->first();
            // return $check;
            if(isset($check)){
            DB::table("$m_emp_day_policy_table")
            ->where('employee_id',$value->employee_id)
            ->where('attn_date',$attn_date)
            ->update($data);
            ...
            ->where('employee_id',$value->employee_id)
            ->update([
            'emp_day_policy_status'=>'0'
            ]);

            }else{
            DB::table("$m_emp_day_policy_table")->insert($data);
    ...
    public function HrmEmpDayShiftNoChange()
    {
        ...
        return view('hrm.emp_day_shift_no_change',compact('user_company','ci_placeofposting'));
    ...
    public function HrmEmpDayShiftFinalnoChange(Request $request)
    {
        ...
        //table
        $year = date('Y', strtotime($request->txt_atten_date));
        $month = date('m', strtotime($request->txt_atten_date));
        $m_emp_day_policy_table = "pro_emp_day_policy_$year$month";
        if (Schema::hasTable("$m_emp_day_policy_table")) {
        } else {
            Schema::create("$m_emp_day_policy_table", function (Blueprint $table) {
...
// dd($request->cbo_placeofposting_id);
       ...
       // $txt_placeofposting_name = $m_placeofposting->placeofposting_name;
        ...
        return redirect()->route('emp_day_shift_no_change')->with('success',"$request->txt_atten_date |  $txt_company_name | Day Shift Change Successfully!");

    }

    public function HrmEmpMonthShiftNoChange()
    {
        $m_user_id=Auth::user()->emp_id;
        ...
        $m_placeofposting=DB::table('pro_placeofposting')
        ->Where('valid','1')
        ->Where('sub_posting_status','1')
        ->orderBy('placeofposting_id','asc')
        ->get();

        return view('hrm.emp_month_shift_no_change',compact('user_company','m_placeofposting'));

    }

    public function HrmEmpMonthShiftFinalnoChange(Request $request)
    {
        $rules = [
            'txt_month' => 'required',
            'cbo_company_id' => 'required',
        ...
        //table
        $year = date('Y', strtotime($txt_frist_date));
        $month = date('m', strtotime($txt_frist_date));
        $m_emp_day_policy_table = "pro_emp_day_policy_$year$month";
        if (Schema::hasTable("$m_emp_day_policy_table")) {
        } else {
            Schema::create("$m_emp_day_policy_table", function (Blueprint $table) {
        ...
        $txt_total=30;
        for($d=0; $d<$txt_total; $d++)

        {
            $atdate = date('Y-m-d', strtotime($txt_frist_date.' + '.$d.' days'));
            ...
            $prweekday = date('l', strtotime($atdate));
       ...
       // $txt_placeofposting_name = $m_placeofposting->placeofposting_name;
        ...
        return redirect()->route('emp_month_shift_no_change')->with('success',"$request->txt_month |  $txt_company_name | Day Shift Change Successfully!");

    }

    public function RptEmpDayShift()
    {
        $m_user_id=Auth::user()->emp_id;
        ...
        return view('hrm.rpt_emp_day_shift',compact('user_company','m_placeofposting','m_section'));

    }

     //Daily Punch report 02
    public function hrmbackdaily_punch02()
    {
        $m_user_id = Auth::user()->emp_id;
        $user_company = DB::table("pro_user_company")
            ->join("pro_company", "pro_user_company.company_id", "pro_company.company_id")
        ...
        $m_user_posting = DB::table("pro_user_posting")
            ...
            ->Where('pro_user_posting.employee_id', $m_user_id)
            ->get();
        return view('hrm.daily_punch_02', compact('user_company', 'm_user_posting'));
    }

    public function hrmbackdaily_punch_report02(Request $request)
    {
        // return $request;
        $rules = [
            'cbo_company_id' => 'required|integer|between:1,10000',
        ...
        $customMessages = [
            ...
            'cbo_company_id.between' => 'Select Company.',
            'cbo_posting.required' => 'Select Posting.',
            'txt_date.required' => 'Select form date.',
        ...
        //
        $txt_frist_date = $request->txt_date;
        $company_id = $request->cbo_company_id;
        $placeofposting_id = $request->cbo_posting;
        $placeofposting_sub_id = $request->cbo_sub_posting;
        $txt_month1 = date("m", strtotime($txt_frist_date));
        ...
        if (Schema::hasTable("$m_tmp_log1")) {
            //get employee id
            if ($placeofposting_sub_id == 0) {
                $m_employee_id = DB::table('pro_employee_info')
                    ->where('working_status', '1')
                    ->where('ss', '1')
                    ->where('company_id', $company_id)
                    ->where('placeofposting_id', $placeofposting_id)
            ...
            //
            $m_tmp_log = DB::table("$m_tmp_log1")
                ->whereIn('emp_id', $m_employee_id)
                ->where('logdate', $txt_frist_date)
                ->get();
            if ($m_tmp_log === null) {
                return redirect()->back()->withInput()->with('warning', 'Data Not Found!!');
            } else {

                return view('hrm.emp_punch_report_02', compact('m_tmp_log', 'user_company', 'm_user_posting', 'txt_frist_date', 'company_id'));
            } //if ($m_from_date === null)
        } else { //if (Schema::hasTable("$m_tmp_log1"))
            return redirect()->back()->withInput()->with('warning', 'Data Not Found!!');
        }
    ...
    public function hrmbackleave_application_list02()
    {
        $m_user_id = Auth::user()->emp_id;
        $user_company = DB::table("pro_user_company")
            ->join("pro_company", "pro_user_company.company_id", "pro_company.company_id")
        ...
        return view('hrm.leave_application_list_02', compact('user_company', 'm_user_posting'));
    }
    public function hrmbackleave_application_details02(Request $request)
    {
        $rules = [
            'cbo_employee_id' => 'required',
            'txt_month' => 'required',
        ...
        $m_user_id = Auth::user()->emp_id;
        $m_leave_year = date("Y", strtotime($request->txt_month));
        ...
        $m_level_step = DB::table('pro_level_step')
            ->join("pro_employee_info", "pro_level_step.report_to_id", "pro_employee_info.employee_id")
            ->select("pro_level_step.*", "pro_employee_info.employee_name")
            ->Where('pro_level_step.employee_id', $employee_id)
            ->Where('pro_level_step.valid', '1')
            ->orderby('pro_level_step.level_step', 'ASC')
            ->get();
        ...
        return view('hrm.leave_application_list_02', compact('m_leave_info_master', 'm_level_step', 'm_user_posting', 'user_company'));
    }

 // Report Movement List
    public function hrmbackMovement_register_list()
    {
        $m_user_id = Auth::user()->emp_id;
        $user_company = DB::table("pro_user_company")
            ->join("pro_company", "pro_user_company.company_id", "pro_company.company_id")
        ...
        return view('hrm.rpt_movement_register_list', compact('user_company', 'm_user_posting'));
    }

    public function hrmbackMovement_register_details(Request $request)
    {
        $rules = [
            'cbo_employee_id' => 'required',
            'txt_month' => 'required',
        ...
        $employee_id = $request->cbo_employee_id;
        $m_user_id = Auth::user()->emp_id;
        $m_form_date =  date("Y-m-01", strtotime($request->txt_month));
        $m_to_date =  date("Y-m-t", strtotime($request->txt_month));


        $m_movement = DB::table('pro_late_inform_master')
        ->leftJoin('pro_employee_info', 'pro_late_inform_master.employee_id', 'pro_employee_info.employee_id')
        ->select('pro_late_inform_master.*', 'pro_employee_info.employee_name')
        ->Where('pro_late_inform_master.valid', '1')
        ->where('pro_late_inform_master.employee_id', $request->cbo_employee_id)
        ->whereBetween('pro_late_inform_master.entry_date', [$m_form_date, $m_to_date])
        ->orderBy('pro_late_inform_master.late_inform_master_id', 'DESC')
        ->get();
        ...
        return view('hrm.rpt_movement_register_list', compact('m_movement', 'm_user_posting', 'user_company'));
    }
    ...
    public function GetEmployeeDayShift($m_company,$m_posting,$m_sub_posting)
    {
        if($m_sub_posting == 0){     
            $data = DB::table('pro_employee_info')
            ->where('working_status', '1')
    ...
    public function GetPosting($m_company)
    {
        $m_user_id=Auth::user()->emp_id;
        $data = DB::table('pro_user_posting')
        ->leftjoin("pro_placeofposting", "pro_user_posting.placeofposting_id", "pro_placeofposting.placeofposting_id")
        ->select("pro_user_posting.*", "pro_placeofposting.placeofposting_name")
        ->Where('pro_user_posting.valid', '1')
        // ->Where('pro_user_posting.company_id', $m_company)
        ->Where('pro_user_posting.employee_id', $m_user_id)
        ->orderBy('pro_user_posting.placeofposting_id', 'asc')
        ->get();
        return json_encode($data);
    }    
    ...
    public function GetSubPostingList($placeofposting_id)
    {
        $data=DB::table('pro_sub_placeofposting')
        ->leftjoin('pro_placeofposting','pro_sub_placeofposting.placeofposting_id','pro_placeofposting.placeofposting_id')
        ->select('pro_sub_placeofposting.*','pro_placeofposting.placeofposting_name')
        ->Where('pro_sub_placeofposting.placeofposting_id',$placeofposting_id)
        ->Where('pro_sub_placeofposting.valid','1')
        ->get();
        return json_encode($data);
    }
    ...
    public function HrmEmpIdCard($id)
    {
        $employee_id=$id;
        return view('hrm.emp_id_card', compact('employee_id'));
    }
